...
7. ~~**What ND-500 physical `0x96..0xC4` is**~~ — **ANSWERED 2026-08-31 `[V]`, standoff section 115.** It is the **Domain Information Table (PCB) trap-enable block for domain 0**: all twelve writes land exactly on `pcb_ote1/2`, `pcb_cte1/2`, `pcb_mte1/2`, `pcb_temm1/2`, `pcb_tha`, `pcb_tos`, `pcb_ll`, `pcb_hl` (`NDIX-C machine/pcb.h`, MAXSEG 32), and `pcb_pia` at `0xC8` is the first byte past the block. The never-written gap at `0xBA..0xBB` is `pcb_md`/`pcb_ith`, the only single-BYTE fields in the span, which a 4-byte `PHYSWR` cannot express. Our own `CpuND500.Domain.cs` `DIT_*_OFFSET` constants match every one. **So `place-domain` is installing the trap configuration for the domain it is about to run.** The live blocker is now the NEXT question: does the engine LOAD `OTE`/`MTE`/`THA` from the DIT on domain entry? If not, the configuration SINTRAN installs is inert.
