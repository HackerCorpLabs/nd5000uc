...
|---|---|---|---|
| T1 | The octobus `place-domain` swapper stall | **open - root cause identified, fix under measurement** | **ROOT CAUSE (246): an unused operand slot is written as if it were a value.** `Nd500CpuProcessBridge.cs:1737` fills `values[k]` with a placeholder 0 when the operand ADDRESS is 0 (its own comment: *"Address 0 = missing/unused slot"*); the answer loop then writes that placeholder into the message. The swapper's `MON 377B` has TWO operand addresses with `argCount=4`, so slots 2-3 are unused - and slot 2 is `HSWPI`/`SWPINFO`, which `LNEWSWAP` branches on (`MP-P2-N500.NPL:933`, `IF D><0`). Chain: conversion correct (240) -> writer named by stack trace (241) -> slot layout correct on BOTH sides (242) -> classic does the same write 187x and completes (243) -> causality proven by suppression, STALL->OK (245) -> control run confirms the prediction (246a). **Fix: withhold the value write when the operand address is 0**, `argCount` untouched (SINTRAN reads it as `NUMPA`); blast radius checked on both bridges. RUNNING run247. **Bar for success is NOT "place-domain OK" - it is `micfu 24B` and `restarts` staying non-zero**, because the suppression probe reached OK with `restarts=0/0` and nothing forwarded. |
