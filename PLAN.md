...
| T4 | The cpu-stat domain executes with the SWAPPER's registers | **open - diagnosed, standoff 261** | The MMU is RIGHT: `reason=capability is ZERO - segment 1 is not in domain 0 (program capability table, CED=0, PS=10)`. The give-away is the operand: `B=0x08024278 R=0x08024300` are the **swapper's** registers, and `ea=0x0802428C` is literally operand 3 of the swapper's own `MON 377B`. That exact signature (same B/R, `PS=10`, `CED=0`) is already written in `SwitchToProcessIfNeeded`'s comment as a measured mixed-state fault whose cause it left open - now settled: **a switch moves some state and not the rest.** `CONTEXT SWITCH` appears **0** times in run258 because `OnStartProcessND5000` calls `StartProcessFromContextBlock` directly and never goes through `SwitchToProcessIfNeeded`. NEXT: compare what the domain's context block holds for `B`/`R` against what the CPU holds after the start - same file and same family as T1. |
